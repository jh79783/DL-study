...
따라서 커널은 입력 데이터의 채널수와 같은 차원수를 갖고있어야 한다. 또한 출력 채널수만큼 만들어야 하기 때문에 결국 커널은 (입력 영상 가로, 세로, 채널수, 출력 영상의 채널수) 를 갖고있는 4차원 구조가 되어야 한다.
...
$$
미분과정에서 $\bold{W}^{(I)}_{k,i,j}$와 독립적인 항은 모두 0이 되어 p=i, q=i인 항만 고려하면 식은 다음과 같다.
$$
